...
Метод `m4()` для вставки строк использует средства ORM (Hibernate). И, наконец, в методе `m5()` вставка осуществляется с использованием класса `JdbcTemplate` из фреймворка Spring. 
...
На графиках приняты следующие условные обозначения:
    ...
    <Hibernate> - вставка с использованием Hibernate;
    <JdbcTempl> - вставка с использованием JdbcTemplate.

Измеренный интервал учитывает время выполнения команд `createStatement()` и `prepareStatement()`, а также команды`commit()`. Соответствующие данные для СУБД H2 показаны на Рисунках 2, 3.
